...
<Info>
  The `adUnitid` parameter can also be used to manage creation and destruction of an ad instance. If
  `adUnitid` is set or changed, new ad instance will be created and previous ad instance will be
  destroyed if exists. If `adUnitid` is set to `null`, no ad instance will be created and previous
  ad instance will be destroyed if exists.
</Info>
...
| :------------- | :--------------------------------- | :----------------------------------------------------------------------------------------------------------------- |
| isLoaded       | boolean                            | Whether the ad is loaded and ready to be shown to the user. Automatically set to false when the ad was shown.      |
...
<Info>
  Note that `isOpened` value remains `true` even after the ad is closed. The value changes to
  `false` when ad is initialized via calling `load()`. To determine whether the ad is currently
  showing, use `isShowing` value.
</Info>
